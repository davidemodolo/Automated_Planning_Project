(define (domain problem2)
    ...
    (:action load_box1_on_carrier
        ...
        :precondition (and (location_carrier ?c ?l) (location_box ?b ?l) (location_robot ?r ?l) (not (box_loaded ?b)) (not(carrier_has_box1 ?c)))
        :effect (and (box_on_carrier ?b ?c) (box_loaded ?b) (carrier_has_box1 ?c) (carrier_loaded_1 ?c ?b))
    )

    ; now the unload
    (:action unload_box1_from_carrier
        :parameters (?c - carrier ?b - box ?l - location ?r - robotic_agent)
        :precondition (and (location_carrier ?c ?l) (location_box ?b ?l) (box_on_carrier ?b ?c) (box_loaded ?b) (location_robot ?r ?l) (carrier_loaded_1 ?c ?b) (carrier_has_box1 ?c))
        :effect (and (not (box_on_carrier ?b ?c)) (not (box_loaded ?b)) (location_box ?b ?l))
    )

    (:action load_box12_on_carrier
        :parameters (?c - carrier ?b - box ?l - location ?r - robotic_agent)
        :precondition (and (location_carrier ?c ?l) (location_box ?b ?l) (location_robot ?r ?l) (not (box_loaded ?b)) (carrier_has_box1 ?c) (not(carrier_has_box2 ?c)))
        :effect (and (box_on_carrier ?b ?c) (box_loaded ?b) (carrier_has_box2 ?c) (carrier_loaded_2 ?c ?b))
    )
